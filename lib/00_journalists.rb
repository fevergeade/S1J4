...
print "il y a "
print "#{Array_journalistes.select { |handle| handle[1] == handle[1].upcase}.size}"
puts " dont la 1er lettre est une majuscule"

puts "Trie la liste de handle par ordre alphabétique"
puts Array_journalistes.sort_by {|handle| handle.upcase}

puts "Trie la liste de handle par taille des handle (les plus petits en premiers, les plus grands après)"
puts Array_journalistes.sort_by {|handle| handle.length}
